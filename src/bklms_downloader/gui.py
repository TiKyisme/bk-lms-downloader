...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            outer,
            text=(
                "Tải tài liệu BK-LMS và tự gom vào vài folder dễ tìm: "
                "Bài giảng, Lab, Bài tập, Tài liệu tham khảo..."
            ),
        ...
        ttk.Checkbutton(
            options,
            text="Đi theo course học liệu liên kết",
            variable=self.linked_var,
        ).grid(row=0, column=0, sticky="w")
        ttk.Label(
            options,
            text=(
                "Video luôn được bỏ qua. Output được làm phẳng để "
                "không tạo nhiều tầng folder."
            ),
        ...
        ttk.Label(
            outer,
            text=(
                "Đăng nhập diễn ra trên website BK-LMS trong Chrome; "
                "ứng dụng không hỏi hoặc lưu mật khẩu."
            ),
                ...
                self.events.put(
                    {
                        "event": "login_ready",
                        "message": (
                            "Chrome đã mở. Hãy đăng nhập BK-LMS "
                            "trong Chrome."
                        ),
                    }
                )
    ...
    def _use_current_course(self):
        if self.driver is None:
            messagebox.showwarning(
                "Chưa mở Chrome",
                "Hãy bấm 'Mở Chrome để đăng nhập' trước.",
            )
            return
        ...
        try:
            ...
            if not is_course_url(url):
                messagebox.showwarning(
                    "Chưa ở trang course",
                    (
                        "Hãy mở một course BK-LMS trong Chrome "
                        "rồi thử lại."
                    ),
                )
                return
    ...
    def _start_download(self):
        ...
        if not is_course_url(course_url):
            messagebox.showwarning(
                "Course URL",
                "URL course BK-LMS không hợp lệ.",
            )
            return
        ...
        self._log(
            "Layout: COMPACT "
            "(Bài giảng / Lab / Bài tập / ...)"
        )
        self._log(
            "Video: SKIP "
            "(không hỗ trợ tải video)"
        )
        ...
        def worker():
            try:
                ...
                result = downloader.crawl_course(
                    course_url,
                    output,
                    depth=0,
                )
    ...
    def _open_output(self):
        path = (
            self.last_output
            or Path(self.output_var.get())
        )
        if not path.exists():
            messagebox.showwarning(
                "Thư mục",
                "Chưa tìm thấy thư mục kết quả.",
            )
            return
