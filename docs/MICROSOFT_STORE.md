...
```text
Application / Git tag: 1.1.3 / v1.1.3
Microsoft Store MSIX:  1.1.3.0
```

Do not reuse the packaging-only `1.1.1.0` version and do not create versions
such as `1.1.3.1`. Before preparing an MSIX, run:

```powershell
python tools/validate_versions.py --tag v1.1.3 --msix-version 1.1.3.0
```
