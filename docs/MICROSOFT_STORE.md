...
```text
Application / future Git tag: 1.2.0 / v1.2.0
Microsoft Store MSIX:       1.2.0.0
```

Do not reuse the packaging-only `1.1.1.0` version and do not create versions
such as `1.2.0.1`. Before preparing the next MSIX, run:

```powershell
python tools/validate_versions.py --tag v1.2.0 --msix-version 1.2.0.0
```
